# +==╡The_3_of_Swords_Reversed>　逆位寶劍三
🌏 I'm From [Taiwan](https://en.wikipedia.org/wiki/Taiwan)~  
🎯 [CSIE(Computer Science and Information Engineering)](https://zh.wikipedia.org/wiki/%E8%B3%87%E8%A8%8A%E5%B7%A5%E7%A8%8B%E5%AD%B8%E7%B3%BB) students  
📠 Discord：[Maizuru#0678](http://discordapp.com/invite/BtTUkY8)
### \[Language]  
// _Sort by learning order_  
// _☆ is the degree of preference_
  
1. **[Visual Basic(.NET)](https://en.wikipedia.org/wiki/Visual_Basic_.NET)**  
``［★★★☆☆］``   
2. **[C](https://en.wikipedia.org/wiki/C_\(programming_language\))**  
``［★★★★☆］``  
1. **[Visual Basic(VB6.0)](https://en.wikipedia.org/wiki/Visual_Basic_(classic))**  
``［★☆☆☆☆］``   
3. **[Html](https://en.wikipedia.org/wiki/HTML)&[Css](https://en.wikipedia.org/wiki/CSS)**  
``［★★★☆☆］``  
4. **[JavaScript](https://en.wikipedia.org/wiki/JavaScript)**  
``［★★☆☆☆］``  
5. **[PHP](https://en.wikipedia.org/wiki/PHP)**  
``［★★★☆☆］``   
6. **[C#](https://en.wikipedia.org/wiki/C_Sharp_\(programming_language\))**  
``［★☆☆☆☆］``  
7. **[C++](https://en.wikipedia.org/wiki/C++)**  
``［★★☆☆☆］``  
8. **[Masm(Assembly language)](https://en.wikipedia.org/wiki/Microsoft_Macro_Assembler)**  
``［★☆☆☆☆］``  
9. **[Python](https://en.wikipedia.org/wiki/Python_\(programming_language\))**  
``［★★☆☆☆］``  



